...
export interface MiddlewareData<Config extends object = {}, Context extends object = {}> {
  ...
  /** The Code to run when an HTTP Request Triggers */ httpEvent?: (localContext: Context, stop: StopFn, ctr: HTTPRequestContext, ctx: InternalContext, ctg: GlobalContext) => RealAny
  /** The Code to run when a WebSocket Connects */ wsConnectEvent?: (localContext: Context, stop: StopFn, ctr: WebSocketConnect, ctx: InternalContext, ctg: GlobalContext) => RealAny
  /** The Code to run when a WebSocket Recieves a Message */ wsMessageEvent?: (localContext: Context, stop: StopFn, ctr: WebSocketMessage, ctx: InternalContext, ctg: GlobalContext) => RealAny
  /** The Code to run when a WebSocket Closes */ wsCloseEvent?: (localContext: Context, stop: StopFn, ctr: WebSocketClose, ctx: InternalContext, ctg: GlobalContext) => RealAny
}

type RealAny = Promise<any> | any
type StopFn = () => void
