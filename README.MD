<h1 align="center">Welcome to rjweb-server v4 👋</h1>
<div align="center">
...
server.prefix('/')
  .add('GET', '/hello', async(ctr: HTTPRequestContext) => {
...
```ts
import { Server, HTTPRequestContext, RouteFile } from "rjweb-server"
interface Custom {
  count: number
}
...
server.prefix('/')
  ...
  .add('POST', '/hello', async(ctr: Ctr<{ name?: string }>) => {
...
Function File
```ts
import { HTTPRequestContext, RouteFile } from "rjweb-server"
...
declare module "rjweb-server" {
  export interface HTTPRequestContext extends MiddlewareToHTTPContext<[ AdditionalProps1, AdditionalProps2 ]> {}
}
```
...
server.middleware(someMiddleware())
server.middleware(someOtherMiddleware())
...
const server = new Server({
  ...
  port: 5000, // The Port which the Server runs on
  proxy: true, // If enabled, alternate IPs will be shown
  contentTypes: { // List of File endings and content types to add
