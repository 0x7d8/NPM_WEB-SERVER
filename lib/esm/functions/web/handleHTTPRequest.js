  ...
  ctx.events.once("startRequest", async () => {
    ...
    for (let staticNumber = 0; staticNumber <= ctg.routes.static.length - 1; staticNumber++) {
      ...
      if (!ctx.url.pathname.startsWith(url2.path))
        continue;
      const urlPath = pathParser(ctx.url.pathname.replace(url2.path, "")).substring(1);
      ...
      if (url2.data.hideHTML) {
        if (await fileExists(url2.location + "/" + urlPath + "/index.html"))
          foundStatic(path.resolve(url2.location + "/" + urlPath + "/index.html"), url2);
        else if (await fileExists(url2.location + "/" + urlPath + ".html"))
          foundStatic(path.resolve(url2.location + "/" + urlPath + ".html"), url2);
        else if (await fileExists(url2.location + "/" + urlPath))
          foundStatic(path.resolve(url2.location + "/" + urlPath), url2);
      } else if (await fileExists(url2.location + "/" + urlPath))
        foundStatic(path.resolve(url2.location + "/" + urlPath), url2);
    }
    if (ctg.options.dashboard.enabled && (ctx.url.pathname === pathParser(ctg.options.dashboard.path) || ctx.url.pathname === pathParser(ctg.options.dashboard.path) + "/stats")) {
      ctx.execute.route = {
        ...
        code: async (ctr2) => await statsRoute(ctr2, ctg, ctx),
        data: {
          validations: []
        }
      };
    ...
    let ctr = {
      ...
      printFile(file, options = {}) {
        ...
        if (addTypes && !contentType)
          ctr.setHeader("Content-Type", handleContentType(file, ctg));
        else if (contentType)
          res.setHeader("Content-Type", contentType);
    ...
    if (!ctx.execute.dashboard)
      errorStop = await handleEvent("httpRequest", ctr, ctx, ctg);
    if (errorStop)
      return;
    ...
    if (ctx.execute.exists && !errorStop) {
      if (ctx.execute.route.type === "static") {
        if (ctx.execute.route.data.addTypes)
          res.setHeader("Content-Type", handleContentType(ctx.execute.file, ctg));
        ctx.continue = false;
        let stream, errorStop2 = false;
