  ...
  ctx.events.once("startRequest", async () => {
    ...
    for (let staticNumber = 0; staticNumber <= ctg.routes.static.length - 1; staticNumber++) {
      ...
      if (file && !file.includes("."))
        file += "/";
      if (url2.data.hideHTML && !file.endsWith(".html") && fs.existsSync(path.resolve(url2.location + "/" + file)))
        foundStatic(path.resolve(url2.location + "/" + file), url2);
      if (url2.data.hideHTML && file === "" && url2.data.hideHTML && fs.existsSync(path.resolve(url2.location + "/" + file + ".html")))
        foundStatic(path.resolve(url2.location + "/" + file + ".html"), url2);
      if (url2.data.hideHTML && fs.existsSync(path.resolve(url2.location + "/" + file + "index.html")))
        foundStatic(path.resolve(url2.location + "/" + file + "index.html"), url2);
    }
