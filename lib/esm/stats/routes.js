...
async function statsRoute(ctr2, ctg, ctx, options, routes) {
  const path = ctr2.url.path.replace(pathParser(options.dashboard.path, false), "") || "/";
  switch (path) {
    case "/":
      const dashboard = (await fs.readFile(`${__dirname}/index.html`, "utf8")).replaceAll("/rjweb-dashboard", pathParser(options.dashboard.path)).replace("VERSION 1.1.1", packageJSON.version);
