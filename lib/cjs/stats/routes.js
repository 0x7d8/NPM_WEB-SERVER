...
var import_routeList = require("../classes/routeList");
var import_package = __toESM(require("../package.json"));
var fs = __toESM(require("fs/promises"));
var os = __toESM(require("os"));
const coreCount = os.cpus().length;
async function statsRoute(ctr2, ctg, ctx, options, routes) {
  const path = ctr2.url.path.replace((0, import_routeList.pathParser)(options.dashboard.path, false), "") || "/";
  switch (path) {
    case "/":
      const dashboard = (await fs.readFile(`${__dirname}/index.html`, "utf8")).replaceAll("/rjweb-dashboard", (0, import_routeList.pathParser)(options.dashboard.path)).replace("VERSION 1.1.1", import_package.default.version);
