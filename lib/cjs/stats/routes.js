...
async function statsRoute(ctr, ctg, ctx) {
  const path = ctr.url.path.replace((0, import_router.pathParser)(ctg.options.dashboard.path, false), "") || "/";
  switch (path) {
    case "/":
    ...
    case "/stats":
      ...
      const staticFiles = await new Promise(async (resolve) => {
        ...
        for (let staticNumber = 0; staticNumber <= ctg.routes.static.length - 1; staticNumber++) {
          staticFiles2 += (await (0, import_getAllFiles.getAllFiles)(ctg.routes.static[staticNumber].location)).length;
        }
