...
async function statsRoute(ctr2, ctg, ctx, options, routes) {
  ...
  switch (path) {
    case "/":
      const dashboard = (await fs.readFile(`${__dirname}/index.html`, "utf8")).replaceAll("/rjweb-dashboard", (0, import_routeList.pathParser)(options.dashboard.path)).replace("VERSION 1.1.1", import_pckg.version);
