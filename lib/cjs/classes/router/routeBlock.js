...
var import_require_esm_in_cjs = __toESM(require("require-esm-in-cjs"));
...
class RouteBlock {
  ...
  */
  loadCJS(folder) {
    ...
    for (const file of files) {
      const route = require(import_path.default.resolve(file));
  ...
  */
  loadESM(folder) {
    const files = (0, import_getAllFiles.getAllFilesFilter)(folder, "js");
    for (const file of files) {
      const route = (0, import_require_esm_in_cjs.default)(import_path.default.resolve(file));
      if (route && !("path" in route) || !("method" in route) || !("code" in route))
        continue;
      if (!import_methods2.default.includes(route.method))
        throw TypeError(`No Valid Request Type: ${route.method}, Possible Values: ${import_methods2.default.join(", ")}`);
      this.routes.push({
