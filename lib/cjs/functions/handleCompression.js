...
function handleCompression(ctr2, ctx, ctg) {
  if (!ctx.compressed && !ctr2.rawRes.headersSent && String(ctr2.headers.get("accept-encoding")).includes(import_handleCompressType.CompressMapping[ctg.options.compression])) {
    ctr2.rawRes.setHeader("Content-Encoding", import_handleCompressType.CompressMapping[ctg.options.compression]);
