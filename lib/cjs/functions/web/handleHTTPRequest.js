  ...
  ctx.events.once("startRequest", async () => {
    ...
    for (let staticNumber = 0; staticNumber <= ctg.routes.static.length - 1; staticNumber++) {
      ...
      if (ctx.execute.exists)
        break;
      let file = ctx.url.pathname.replace(url2.path, "");
      if (file && !file.includes("."))
        file += "/";
      if (url2.data.hideHTML && !file.endsWith(".html") && import_fs.default.existsSync(import_path.default.resolve(url2.location + "/" + file)))
        foundStatic(import_path.default.resolve(url2.location + "/" + file), url2);
      if (url2.data.hideHTML && file === "" && url2.data.hideHTML && import_fs.default.existsSync(import_path.default.resolve(url2.location + "/" + file + ".html")))
        foundStatic(import_path.default.resolve(url2.location + "/" + file + ".html"), url2);
      if (url2.data.hideHTML && import_fs.default.existsSync(import_path.default.resolve(url2.location + "/" + file + "index.html")))
        foundStatic(import_path.default.resolve(url2.location + "/" + file + "index.html"), url2);
    }
    for (let urlNumber = 0; urlNumber <= ctg.routes.normal.length - 1; urlNumber++) {
      ...
      if (ctg.options.dashboard.enabled && (ctx.url.pathname === (0, import_router.pathParser)(ctg.options.dashboard.path) || ctx.url.pathname === (0, import_router.pathParser)(ctg.options.dashboard.path) + "/stats")) {
        ctx.execute.route = {
          ...
          code: async (ctr2) => await (0, import_routes.default)(ctr2, ctg, ctx, ctg.routes.normal.length),
          data: {
            addTypes: false,
            authChecks: []
          }
    ...
    let ctr = {
      ...
      printFile(file, localOptions) {
        ...
        if (addTypes && !contentType)
          ctr.setHeader("Content-Type", (0, import_handleContentType.default)(ctx.execute.route.path));
        else if (contentType)
          res.setHeader("Content-Type", contentType);
    ...
    if (ctx.execute.exists && !errorStop) {
      if (!ctx.execute.static && "code" in ctx.execute.route) {
        ...
        if (ctx.execute.route.data.addTypes)
          res.setHeader("Content-Type", (0, import_handleContentType.default)(ctx.execute.file));
        ctx.continue = false;
        const filePath = import_path.default.resolve(ctx.execute.file);
        let stream, errorStop2 = false;
        if (ctg.options.compression && String(ctr.headers.get("accept-encoding")).includes(import_handleCompressType.CompressMapping[ctg.options.compression])) {
          ctr.rawRes.setHeader("Content-Encoding", import_handleCompressType.CompressMapping[ctg.options.compression]);
          ctr.rawRes.setHeader("Vary", "Accept-Encoding");
          const compression = (0, import_handleCompressType.default)(ctg.options.compression);
          try {
