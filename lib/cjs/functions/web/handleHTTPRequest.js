  ...
  if (ctg.options.body.enabled)
    req.on("data", (data) => {
      ...
      if (ctx.body.raw.byteLength >= ctg.options.body.maxSize * 1e6) {
        res.statusCode = 413;
        ctx.continue = false;
        const result = await (0, import_parseContent.default)(ctg.options.body.message);
  ...
  ctx.events.once("startRequest", async () => {
    ...
    let ctr = {
      ...
      print(message, options = {}) {
        var _a;
        const contentType = (_a = options == null ? void 0 : options.contentType) != null ? _a : "";
        ctx.events.emit("noWaiting");
        ctx.waiting = true;
        (async () => {
          let result;
          try {
            result = await (0, import_parseContent.default)(message);
          } catch (err) {
      ...
      printFile(file, options = {}) {
        ...
        const contentType = (_b = options == null ? void 0 : options.contentType) != null ? _b : "";
        ...
        if (ctr.headers.get("accept-encoding", "").includes(import_handleCompressType.CompressMapping[ctg.options.compression])) {
          res.setHeader("Content-Encoding", import_handleCompressType.CompressMapping[ctg.options.compression]);
          ...
          try {
            ...
            errorStop2 = true;
            (0, import_handleEvent.default)("runtimeError", ctr, ctx, ctg, err);
          }
          if (errorStop2)
            return;
          ...
          compression.on("data", (content) => {
            ...
            if (cache) {
              const oldData = (_a2 = ctg.cache.files.get(`file::${ctg.options.compression}::${file}`)) != null ? _a2 : Buffer.from("");
              ctg.cache.files.set(`file::${ctg.options.compression}::${file}`, Buffer.concat([oldData, content]));
            }
          });
          compression.once("close", () => {
            ctx.events.emit("noWaiting");
      ...
      printStream(stream, options = {}) {
        var _a, _b;
        const endRequest = (_a = options == null ? void 0 : options.endRequest) != null ? _a : true;
        const destroyAbort = (_b = options == null ? void 0 : options.destroyAbort) != null ? _b : true;
        ctx.waiting = true;
        const dataListener = async (data) => {
          try {
            data = (await (0, import_parseContent.default)(data)).content;
          } catch (err) {
            return ctx.handleError(err);
          }
        ...
        if (destroyAbort)
          ctx.events.once("endRequest", () => stream.destroy());
        stream.on("data", dataListener).once("close", closeListener);
        ctx.events.once("endRequest", () => stream.removeListener("data", dataListener).removeListener("close", closeListener));
        return ctr;
      }
    };
    let errorStop = false;
    ...
    if (ctg.middlewares.length > 0) {
      ...
      for (let validateIndex = 0; validateIndex <= ctg.middlewares.length - 1; validateIndex++) {
        ...
        if (!doContinue && runError) {
          errorStop = true;
          (0, import_handleEvent.default)("runtimeError", ctr, ctx, ctg, runError);
          break;
        } else if (!doContinue) {
    ...
    if (!ctx.execute.dashboard)
      errorStop = await (0, import_handleEvent.default)("httpRequest", ctr, ctx, ctg);
    if (errorStop)
      return;
    ...
    if (ctx.execute.exists && !errorStop) {
      ...
      (0, import_handleEvent.default)("http404", ctr, ctx, ctg);
      if (ctx.waiting)
        await new Promise((resolve) => {
          ctx.events.once("noWaiting", () => resolve(false));
        });
