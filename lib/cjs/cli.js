...
if (args.includes("-v") || args.includes("--version")) {
  ...
  console.log(`${import_pckg.version}`);
} else if (!isHelp() && import_fs.default.existsSync(import_path.default.join(process.cwd(), args[0]))) {
  ...
  for (const option of args.slice(1)) {
    ...
    if (key === "cors")
      webserverOptions.cors = true;
    if (key === "proxy")
      webserverOptions.proxy = true;
    if (key === "dashboard")
      webserverOptions.dashboard.enabled = true;
    if (key === "404")
  ...
  server.path(
    "/",
    (r) => r.static(import_path.default.join(process.cwd(), args[0]), { hideHTML })
  );
  if (notFoundPath)
    server.event("http404", (ctr) => {
      return ctr.status(404).printFile(import_path.default.join(process.cwd(), notFoundPath));
    });
  ...
  console.log(`${colors.fg.blue}[INF] ${colors.fg.red}An Error occurred!`);
  console.log(`${colors.fg.red}[ERR] ${colors.fg.gray}Folder ${colors.fg.red, colors.underscore}${import_path.default.join(process.cwd(), args[0])}${colors.reset} couldnt be found`);
} else {
