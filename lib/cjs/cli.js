...
if (args.includes("-v") || args.includes("--version")) {
  ...
  webserverOptions.routes = new import__.default.routeList();
  webserverOptions.routes.static("/", path.join(process.cwd(), args[0]), { remHTML, addTypes });
  if (notFoundPath)
    webserverOptions.routes.event("notfound", async (ctr) => {
  ...
  import__.default.initialize(webserverOptions).setRoutes(webserverOptions.routes).start().then((res) => {
